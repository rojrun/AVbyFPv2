...
import {Button, Card, DisplayText, Page, Layout, Stack} from '@shopify/polaris';
...
class Cart extends React.Component {
  ...
  subtractQuantity = (quantity, index) => {
    console.log("subtractQuantity clicked");
    quantity--;
    if (quantity < 0) {
      quantity = 0;
    }
    this.setState({cart: this.state.cart[index].quantity});
  ...
  addQuantity = (quantity, index) => {
    ...
    quantity++;
    if (quantity > this.state.cart[index].product.node.totalInventory) {
      quantity = this.state.cart[index].product.node.totalInventory;
    }
    this.setState({cart: this.state.cart[index].quantity});
    store.set('cart', this.state.cart);
  }
                          ...
                          <DisplayText size="small">Quantity:&nbsp;
                            <span>
                              <Button
                                ...
                                onClick={() => this.subtractQuantity(quantity, index)}
                              ></Button>
                            </span>
                            <span>
                              &nbsp;{quantity}&nbsp;
                            ...
                            <span>
                              <Button
                                ...
                                onClick={() => this.addQuantity(quantity, index)}
                              ></Button>
                            </span>
                          </DisplayText>
                          <br/>
